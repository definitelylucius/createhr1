    ...
    <div class="flex items-center space-x-4 relative">
        ...
        <div class="relative">
            <button type="button" onclick="toggleNotificationDropdown()" class="p-2 rounded-full hover:bg-gray-200 relative">
                ...
                <span id="notification-badge" class="absolute top-0 right-0 block h-3 w-3 rounded-full bg-red-500"></span>
            </button>
        ...
        <div class="relative">
            ...
            <div id="settingsDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg z-20">
                ...
                <form method="POST" action="{{ route('logout') }}" onsubmit="event.preventDefault(); this.submit(); window.location.href = '/login';">
    @csrf
            ...
            if (data.length > 0) {
                ...
                notificationList.innerHTML = data.map(n =>
                    `<p class="text-gray-600 p-3 text-sm border-b">
                        <a href="#" onclick="markNotificationsAsRead(event, '${n.id}', '${n.url || '/staff/recruitment/feedback'}')">
                            ${n.message}
                        </a>
                    </p>`
                ).join('');
...
function markNotificationsAsRead(event, id, redirectUrl) {
